...
Run `python -m pytest tests/test_systemone.py tests/test_vlm_engine.py -k "systemone or read_session" -q` and `node tests/system_one_rereads_ui.test.cjs`. The Python cases check the `/v1/systemone` models field by field against the fields and required fields of TypeSafe's OpenAPI 0.2.0, the documented confidence examples, label and slot resolution, chunking, djev's noise-draw policy and its `system_one_rereads_enabled` switch, and the engine's single prefill per read against a fake diffusion model. The Node case checks that the re-read toggle is shown, saved and profiled for diffusion models only. With a DiffusionGemma checkpoint, `OMLX_DIFFUSIONGEMMA_MODEL=/path/to/checkpoint python -m pytest tests/test_systemone.py -m slow` adds the real tokenizer's labels and one real read. For accuracy, run JevBench's public items with its `typesafe` adapter against a local server and compare with its published per-task outcomes.
