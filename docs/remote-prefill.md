...
1. When a request reaches the head of the queue, oMLX checks its local prefix
   ...
   uncached tokens before its generation prompt, oMLX sends the prompt's token
   IDs to vLLM with a handoff ID, asking it to export only what the Mac lacks.
2. The request waits at the head of the queue while vLLM prefills. Requests
   already running keep decoding.
3. oMLX pulls every rank's pages frame by frame. It checks each frame's CRC-32,
   the frame's place in the manifest, and a digest of the prompt the producer
   prefilled. It then frees the pages on the vLLM side.
...
the log: vLLM refusing or timing out, a link going down or reconnecting, a bad
checksum or manifest, or pages for a different prompt. After three failures in
a row remote prefill pauses for a minute.
...
| `OMLX_REMOTE_PREFILL_TIMEOUT` | Seconds for one remote prefill, transfer included (default 600) |
...
- One remote prefill at a time: the request at the head of the queue. Requests
  behind it wait, as they do behind a cache freshness wait.
