...
class OMLXAppDelegate(NSObject):
    ...
    def _check_for_updates(self):
        """Disabled in flyto-mlx: do not poll upstream jundot/omlx releases.

        flyto is a soft-fork on a separate version line (0.4.x) and pulls
        upstream changes via curated cherry-picks (see docs/upstream-sync.md),
        not via the menubar auto-updater. Leaving the check on would surface
        an unwanted "Update Available" banner whenever upstream cuts a tag.
        """
