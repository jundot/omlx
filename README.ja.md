...
- **モデルプロファイル**: 各モデルプロファイルにカスタム設定を設定できます。`:` で区切った形で即席で使用可能です。`(model_name|model_alias):model_profile name` のようなパターンは、選択されたプロファイルでリクエストを処理します。プロファイルが指定されていない場合はデフォルト値を使用します。
