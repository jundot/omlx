...
class TestCacheSettings:
    ...
    def test_legacy_alias_key_still_loads_and_is_written_back_both_ways(self):
        ...
        The supported on-disk legacy spelling is ``gdn_sidecar_precision``.
        ``gdn_sidecar_state_dtype`` is the v0.6.0 spelling that persisted a
        lossy default without recording user intent, so file readers keep
        ignoring it; the alias stays accepted on the API surface, where a
        value is always an explicit selection.
        """
        settings = CacheSettings.from_dict(
            {"gdn_ssd_split_enabled": True, "gdn_sidecar_precision": "rht_int8"}
        )
        ...
        assert stored["gdn_sidecar_precision"] == "rht_int8"
