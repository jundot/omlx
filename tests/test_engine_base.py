...
def test_effective_width_log_includes_the_qwen4_wide_step(caplog):
    """The qwen4_exp wide step only engages after the first chunk, so the probe
    has to sit in the bulk phase or the log under-reports the width (#3903)."""
    from omlx.scheduler import Scheduler

    scheduler = SimpleNamespace(
        config=SchedulerConfig(prefill_step_size=1024),
        model=SimpleNamespace(model_type="qwen4_exp"),
        _glm_dsa_adaptive_prefill=None,
        _minimax_m3_adaptive_prefill=None,
    ...
    scheduler._base_prefill_step_size = (
        lambda processed, remaining: Scheduler._base_prefill_step_size(
            scheduler, processed, remaining
        )
    )

    with caplog.at_level(logging.INFO):
        log_effective_prefill_step_size(scheduler, "qwen4-model")

    assert "configured=1024" in caplog.text
    assert "effective=8192" in caplog.text
