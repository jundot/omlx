...
SOURCE = Path(__file__).resolve().parents[1] / 'omlx/patches/mlx_vlm_qwen4_exp_compat/vendor/mlx_vlm/models/qwen4_exp/language.py'
...
def load_runtime():
    ...
    for node in tree.body:
        if isinstance(node, (ast.FunctionDef, ast.ClassDef)) and (
            node.name.startswith('_ple_') or node.name in {'_SafeTensorMMap', 'DiskBackedShardedEmbedding'}
        ):
            selected.append(node)
        elif isinstance(node, ast.Assign) and any(isinstance(t, ast.Name) and t.id.startswith('_PLE_') and t.id not in {'_PLE_RUNTIME_MODEL_PATH', '_PLE_RUNTIME_MODE'} for t in node.targets):
            selected.append(node)
        elif isinstance(node, ast.If) and 'register_at_fork' in ast.unparse(node.test):
            selected.append(node)
    namespace = dict(os=os, Lock=threading.Lock, RLock=threading.RLock, ThreadPoolExecutor=ThreadPoolExecutor,
                     Path=Path, struct=struct, json=json, mmap=mmap, np=np, math=math,
                     time=time, wait=wait, register_ple_resource=lambda *a, **k: None, logger=logging.getLogger(__name__), mx=NoMLX(),
...
class RuntimePLEForkTests(unittest.TestCase):
    def setUp(self):
        ...
        header = json.dumps({'weight': {'shape': [16, 4], 'dtype': 'F32', 'data_offsets': [0, self.values.nbytes]}}).encode()
        self.path.write_bytes(struct.pack('<Q', len(header)) + header + self.values.tobytes())
    ...
    def test_inherited_locks_future_reject_before_mlx_and_child_close(self):
        ...
        def child():
            ...
            self.assertFalse(e._prefetch_lock.acquire(timeout=.03))
            with self.assertRaises(TimeoutError):
                future.result(timeout=.03)
            for call in (lambda: e.prefetch(object()), lambda: e(object()),
        ...
        np.testing.assert_array_equal(self.reader.rows_np('weight', [2])[0], self.values[[2]])
        e.close()
    ...
    def test_initialized_global_pool_cannot_be_used_by_new_child_reader(self):
        ...
        fork_check(child)
        np.testing.assert_array_equal(self.reader.rows_np('weight', np.arange(16))[0], self.values)
    ...
    def test_close_waits_for_active_reader_without_blocking_other_reader(self):
        ...
        try:
            ...
            np.testing.assert_array_equal(other.rows_np('weight', [1])[0], self.values[[1]])
            other.close()
        finally:
            release.set()
            worker.join(2)
    ...
    def test_child_exported_view_close_retry_does_not_close_reused_fd(self):
        ...
        try:
            ...
            view.release()
        np.testing.assert_array_equal(self.reader.rows_np('weight', [4])[0], self.values[[4]])
        e.close()
    ...
    def test_resource_lock_allows_same_thread_fork_reentry(self):
        def child():
            with NS['_PLE_RESOURCE_LOCK']:
                fork_check(lambda: self.reader.close())
        fork_check(child)
        np.testing.assert_array_equal(self.reader.rows_np('weight', [3])[0], self.values[[3]])
    ...
    def test_failed_page_read_drains_peers_before_close_and_reraises(self):
        ...
        original = self.reader._rows_np_owned
        def read_pages(*args):
            self.reader._prefetch_missing_pages(np.array([0, 1]), 0, NS['_PLE_PAGE_SIZE'])
            return original(*args)
