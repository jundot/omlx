...
def _builtin_price(match: str) -> dict:
    """Look up a builtin default row's price by ``match`` key, for assertions."""
    row = get_pricing_table().lookup(match)
...
def _reset_singletons():
    """Isolate the ServerMetrics / UsageLedger / PricingTable singletons.

    All three are process-wide globals shared with every other test module,
    so each test must start (and leave) them pointing at no persistence
    path. The pricing table still loads its bundled defaults with
    ``base_path=None`` — only the user-overrides file is disabled.
    """
    reset_server_metrics()
    reset_usage_ledger()
    reset_pricing_table(None)
    yield
    reset_server_metrics()
    reset_usage_ledger()
    reset_pricing_table(None)
...
class TestCostEstimation:
    ...
    def test_longest_key_match_wins(self):
        ...
        assert cost == pytest.approx(_builtin_price("gpt-4o-mini")["input"])
        assert cost != pytest.approx(_builtin_price("gpt-4o")["input"])
...
class TestPricingTable:
    """Tests for the data-driven PricingTable (defaults + user overrides)."""

    def test_defaults_load_from_shipped_json(self):
        """The bundled JSON file backs the table, not a hard-coded dict."""
        assert DEFAULT_PRICING_FILE.exists()
        table = PricingTable(base_path=None)
        ...
        assert "claude-3-5-sonnet" in matches

    def test_lookup_preserves_longest_match_semantics(self):
        table = PricingTable(base_path=None)
        assert table.lookup("gpt-4o-mini-2024-07-18") == table.lookup("gpt-4o-mini")
        assert table.lookup("gpt-4o-mini") != table.lookup("gpt-4o")
        assert table.lookup("") is None
        assert table.lookup("mlx-community/my-local-model-4bit") is None

    def test_user_row_overrides_builtin_by_match(self, tmp_path):
        table = PricingTable(base_path=tmp_path)
        table.add_row({"match": "gpt-4o", "input": 1.0, "output": 2.0})

        assert table.lookup("gpt-4o") == {"input": 1.0, "output": 2.0}

        rows = table.rows(include_builtin=True)
        gpt4o_rows = [r for r in rows if r["match"] == "gpt-4o"]
        assert len(gpt4o_rows) == 2  # builtin (overridden) + user
        by_source = {r["source"]: r for r in gpt4o_rows}
        assert by_source["builtin"]["overridden"] is True
        assert by_source["user"]["input"] == 1.0

    def test_user_row_extends_table_with_new_key(self, tmp_path):
        table = PricingTable(base_path=tmp_path)
        table.add_row(
            {"match": "my-custom-model", "input": 5.0, "output": 6.0}
        )
        assert table.lookup("my-custom-model-v2") == {"input": 5.0, "output": 6.0}
        user_rows = table.rows(include_builtin=False)
        assert len(user_rows) == 1
        assert user_rows[0]["match"] == "my-custom-model"
        assert user_rows[0]["overridden"] is False

    def test_add_row_persists_to_user_file(self, tmp_path):
        table = PricingTable(base_path=tmp_path)
        table.add_row({"match": "foo", "input": 1.0, "output": 2.0})

        user_file = tmp_path / "cloud_pricing.json"
        assert user_file.exists()
        data = json.loads(user_file.read_text())
        assert data["rows"] == [{"match": "foo", "input": 1.0, "output": 2.0}]

    def test_add_row_display_name_roundtrips(self, tmp_path):
        table = PricingTable(base_path=tmp_path)
        row = table.add_row(
            {
                "match": "foo",
        ...
        assert row["display_name"] == "Foo Model"
        rows = table.rows(include_builtin=False)
        assert rows[0]["display_name"] == "Foo Model"

    def test_delete_user_row_restores_builtin(self, tmp_path):
        table = PricingTable(base_path=tmp_path)
        original = table.lookup("gpt-4o")
        table.add_row({"match": "gpt-4o", "input": 1.0, "output": 2.0})
        assert table.lookup("gpt-4o") == {"input": 1.0, "output": 2.0}

        assert table.delete_row("gpt-4o") is True
        assert table.lookup("gpt-4o") == original

    def test_delete_builtin_only_row_returns_false(self, tmp_path):
        table = PricingTable(base_path=tmp_path)
        assert table.delete_row("gpt-4o") is False
        assert table.lookup("gpt-4o") is not None

    def test_delete_unknown_row_returns_false(self, tmp_path):
        table = PricingTable(base_path=tmp_path)
        assert table.delete_row("nonexistent-model") is False

    @pytest.mark.parametrize(
        "row",
        [
            {"match": "", "input": 1.0, "output": 2.0},
            {"match": "   ", "input": 1.0, "output": 2.0},
            {"match": "foo", "input": -1.0, "output": 2.0},
            {"match": "foo", "input": 1.0, "output": -2.0},
            {"match": "foo", "input": float("inf"), "output": 2.0},
            {"match": "foo", "input": float("nan"), "output": 2.0},
            {"match": "foo", "input": "not-a-number", "output": 2.0},
            {"match": "foo", "output": 2.0},
            {"match": "foo", "input": 1.0, "display_name": 123},
        ],
    )
    def test_add_row_rejects_invalid_input(self, tmp_path, row):
        table = PricingTable(base_path=tmp_path)
        with pytest.raises(ValueError):
            table.add_row(row)

    def test_add_row_without_base_path_raises(self):
        table = PricingTable(base_path=None)
        with pytest.raises(ValueError):
            table.add_row({"match": "foo", "input": 1.0, "output": 2.0})

    def test_match_key_is_lowercased(self, tmp_path):
        table = PricingTable(base_path=tmp_path)
        table.add_row({"match": "MyModel", "input": 1.0, "output": 2.0})
        assert table.lookup("mymodel-v1") == {"input": 1.0, "output": 2.0}

    def test_direct_file_edit_is_picked_up_without_restart(self, tmp_path):
        """Editing cloud_pricing.json on disk is live, no reset needed."""
        table = PricingTable(base_path=tmp_path)
        assert table.lookup("hand-edited-model") is None

        user_file = tmp_path / "cloud_pricing.json"
        user_file.write_text(
            json.dumps(
                {"rows": [{"match": "hand-edited-model", "input": 9.0, "output": 9.0}]}
            )
        )
        # Bump the mtime forward in case the write above lands within the
        # same filesystem-timestamp tick as PricingTable's initial read.
        current = user_file.stat().st_mtime
        os.utime(user_file, (current + 1, current + 1))

        assert table.lookup("hand-edited-model") == {"input": 9.0, "output": 9.0}

    def test_reset_pricing_table_points_at_new_base_path(self, tmp_path):
        reset_pricing_table(tmp_path)
        get_pricing_table().add_row({"match": "foo", "input": 1.0, "output": 2.0})
        assert (tmp_path / "cloud_pricing.json").exists()

    def test_estimate_api_cost_uses_live_pricing_table(self, tmp_path):
        """Live invalidation: add a row -> estimate_api_cost sees it immediately."""
        reset_pricing_table(tmp_path)
        assert estimate_api_cost("brand-new-model", 1_000_000, 1_000_000) is None

        get_pricing_table().add_row(
            {"match": "brand-new-model", "input": 1.0, "output": 2.0}
        )
        cost = estimate_api_cost("brand-new-model", 1_000_000, 1_000_000)
        assert cost == pytest.approx(3.0)


class TestPricingEndpoints:
    """Tests for GET/POST/DELETE /admin/api/usage/pricing."""

    def test_get_pricing_returns_merged_rows_and_user_file(self, client, tmp_path):
        reset_pricing_table(tmp_path)

        resp = client.get("/admin/api/usage/pricing")
        assert resp.status_code == 200
        body = resp.json()
        assert body["user_file"] == str(tmp_path / "cloud_pricing.json")
        assert len(body["rows"]) == 15
        assert all(r["source"] == "builtin" for r in body["rows"])

    def test_post_adds_user_row(self, client, tmp_path):
        reset_pricing_table(tmp_path)

        resp = client.post(
            "/admin/api/usage/pricing",
            json={"match": "my-model", "input": 1.0, "output": 2.0},
        )
        assert resp.status_code == 200
        assert resp.json()["match"] == "my-model"

        rows = client.get("/admin/api/usage/pricing").json()["rows"]
        user_rows = [r for r in rows if r["source"] == "user"]
        assert len(user_rows) == 1
        assert user_rows[0]["match"] == "my-model"

        cost = estimate_api_cost("my-model", 1_000_000, 1_000_000)
        assert cost == pytest.approx(3.0)

    def test_post_overriding_builtin_marks_it_overridden(self, client, tmp_path):
        reset_pricing_table(tmp_path)

        resp = client.post(
            "/admin/api/usage/pricing",
            json={"match": "gpt-4o", "input": 1.0, "output": 2.0},
        )
        assert resp.status_code == 200

        rows = client.get("/admin/api/usage/pricing").json()["rows"]
        builtin_row = next(
            r for r in rows if r["match"] == "gpt-4o" and r["source"] == "builtin"
        )
        assert builtin_row["overridden"] is True

    def test_post_rejects_invalid_row(self, client, tmp_path):
        reset_pricing_table(tmp_path)

        resp = client.post(
            "/admin/api/usage/pricing",
            json={"match": "", "input": 1.0, "output": 2.0},
        )
        assert resp.status_code == 400

    def test_post_rejects_negative_price(self, client, tmp_path):
        reset_pricing_table(tmp_path)

        resp = client.post(
            "/admin/api/usage/pricing",
            json={"match": "bad-model", "input": -1.0, "output": 2.0},
        )
        assert resp.status_code == 400

    def test_delete_user_row(self, client, tmp_path):
        reset_pricing_table(tmp_path)
        client.post(
            "/admin/api/usage/pricing",
            json={"match": "my-model", "input": 1.0, "output": 2.0},
        ...
        resp = client.delete("/admin/api/usage/pricing/my-model")
        assert resp.status_code == 200

        rows = client.get("/admin/api/usage/pricing").json()["rows"]
        assert not any(r["match"] == "my-model" for r in rows)

    def test_delete_builtin_only_row_is_404(self, client, tmp_path):
        reset_pricing_table(tmp_path)

        resp = client.delete("/admin/api/usage/pricing/gpt-4o")
        assert resp.status_code == 404

    def test_delete_override_restores_builtin_via_api(self, client, tmp_path):
        reset_pricing_table(tmp_path)
        client.post(
            "/admin/api/usage/pricing",
            json={"match": "gpt-4o", "input": 1.0, "output": 2.0},
        )
        assert client.delete("/admin/api/usage/pricing/gpt-4o").status_code == 200

        rows = client.get("/admin/api/usage/pricing").json()["rows"]
        gpt4o_rows = [r for r in rows if r["match"] == "gpt-4o"]
        assert len(gpt4o_rows) == 1
        assert gpt4o_rows[0]["source"] == "builtin"
        assert gpt4o_rows[0]["overridden"] is False
