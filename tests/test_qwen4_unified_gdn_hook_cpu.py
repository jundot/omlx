...
class HookTests(unittest.TestCase):
    def setup_hook(self):
        ...
        q35._target_verify_linears = Mock(
            return_value=[array((1, 3, d)) for d in (10240, 6144, 48, 48)]
        )
        ...
        generic.gdn_prework_fused = Mock(
            return_value=("lazy_q", "lazy_k", "lazy_v", "unused")
        )
        packages = {
            ...
            for name in (
                ...
                "mlx_vlm.models.qwen4_exp.cache",
            )
        }
        ...
        namespace = {
            ...
            "mx": SimpleNamespace(
                ...
                concatenate=lambda *a, **k: "conv_input",
            ),
        }
        exec(
    ...
    def test_admission_declines_width_padding_state_and_unknown_cache(self):
        ...
        cases = [
            (array((1, 8, 2560)), good, "input not"),
            (array((1, 3, 2560)), ns["Cache"]([good[0], None]), "state/parameter"),
        ]
        padded = ns["Cache"](good)
        padded.left_padding = [1]
        ...
        with patch.dict(sys.modules, packages):
            for inputs, cache, reason in cases:
                self.assertTrue(
                    ns["_admission"](module, inputs, None, cache, []).startswith(reason)
                )
    ...
    def test_preserves_full_legacy_sink_and_commits_after_output(self):
        ...
        with patch.dict(sys.modules, packages):
            self.assertEqual(
                ns["try_fused_rms"](module, array((1, 3, 2560)), None, cache, sink),
                "output",
            )
        ...
        self.assertEqual(
            q35._target_verify_linear.call_args.args, ("out", "flat", True)
        )
    ...
    def actual_cache_sources(self):
        ...
        sources = [
            root
            / "omlx/patches/mlx_vlm_qwen4_exp_compat/vendor/mlx_vlm/models/qwen4_exp/cache.py"
        ]
        installed = Path(distribution("mlx-lm").locate_file("mlx_lm/models/cache.py"))
        self.assertTrue(
            installed.is_file(), "CPU ABI gate requires the installed LM cache source"
        )
    ...
    def test_actual_sized_wrapper_preserves_state_slots_size_and_outer_commit(self):
        for source in self.actual_cache_sources():
            with self.subTest(source=str(source)):
                ...
                wrapper = packages["omlx.cache.type_handlers"].SizedArraysCache(
                    inner, 4096
                )
                ...
                with patch.dict(sys.modules, packages):
                    self.assertEqual(
                        ns["try_fused_rms"](
                            module, array((1, 3, 2560)), None, wrapper, sink
                        ),
                        "output",
                    )
                self.assertEqual(
                    inner.state, ["next_conv", "next_state", "ple_history", "ple_conv"]
                )
                ...
                q35._qwen3_5_advance_left_padding_info.assert_called_once_with(
                    wrapper, 3
                )
    ...
    def test_actual_sized_wrapper_rejects_unknown_subclass_nested_and_padding(self):
        ...
        cases = [
            wrapper_cls(object(), 4096),
            wrapper_cls(InnerSubclass(good), 4096),
            WrapperSubclass(good, 4096),
            wrapper_cls(wrapper_cls(good, 4096), 4096),
        ]
        with patch.dict(sys.modules, packages):
            for cache in cases:
                self.assertTrue(
                    ns["_admission"](
                        module, array((1, 3, 2560)), None, cache, []
                    ).startswith("cache is outside")
                )
            for attribute in ("lengths", "left_padding"):
                ...
                self.assertEqual(
                    ns["_admission"](module, array((1, 3, 2560)), None, wrapped, []),
                    "padded cache",
                )
        self.assertFalse(kernel.called)
    ...
    def test_actual_arrays_cache_advance_methods_in_hook_are_noops_for_admitted_lane(
        self,
    ):
        root = SOURCE.parents[2]
        sources = [
        ...
        sources.append(
            Path(distribution("mlx-lm").locate_file("mlx_lm/models/cache.py"))
        )
        for source in sources:
            cls = next(
                n
                for n in ast.parse(source.read_text()).body
                if isinstance(n, ast.ClassDef) and n.name == "ArraysCache"
            )
            method = next(
                n
                for n in cls.body
            ...
            ns["Cache"].advance = advance
            cache = ns["Cache"](
                [array((1, 3, 10240)), array((1, 48, 128, 128), "fp32")]
            )
