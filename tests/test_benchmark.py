...
@pytest.mark.asyncio
async def test_staggered_batch_joins_after_first_token_and_reports_cache():
    import asyncio
    import itertools
    ...
    class Core:
        ...
        async def add_request(self, prompt, sampling_params, skip_cache_store):
            assert not skip_cache_store
            uid = len(self.prompts)
            self.prompts[uid] = prompt
            if uid:
                self.joined.set()
            return uid

        async def stream_outputs(self, uid):
            if uid == 0:
                await asyncio.sleep(0)
                assert len(self.prompts) == 1
                yield SimpleNamespace(completion_tokens=1, finished=False)
                await self.joined.wait()
            yield SimpleNamespace(
                completion_tokens=2, prompt_tokens=4, cached_tokens=2, finished=True
            )

    core = Core()
    clock = itertools.count()
    with patch(
        "omlx.admin.benchmark.time.perf_counter", side_effect=lambda: next(clock)
    ):
        result = await asyncio.wait_for(
            _run_batch_test(
                SimpleNamespace(_engine=core),
                [[1] * 4, [2] * 4],
                4,
                2,
                2,
    ...
    assert result["cached_tokens"] == [2, 2]
    assert result["aggregate_tps"] == pytest.approx(4 / result["e2e_latency_s"])
    assert result["aggregate_tps"] < result["tg_tps"]


@pytest.mark.asyncio
async def test_batch_failure_cancels_other_requests():
    import asyncio
    ...
    class Core:
        ...
        async def abort_request(self, uid):
            self.aborted.append(uid)

    core = Core()
    with pytest.raises(RuntimeError, match="injected"):
        await asyncio.wait_for(
            _run_batch_test(SimpleNamespace(_engine=core), [[1], [2]], 1, 2, 2),
            timeout=2,
        )
    assert sorted(core.aborted) == [1, 2]
