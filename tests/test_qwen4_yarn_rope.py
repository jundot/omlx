...
from mlx_vlm.models.rope_utils import (  # noqa: E402
...
def test_yarn_applies_to_attention_and_shared_indexer():
    ...
    # mscale != 1: the scaling-aware fused kernel takes over — the registry
    # replaces the unscaled compiled applies, so the module stays fused.
    if mx.metal.is_available():
        assert attn.rotary_emb.fused_apply is True
        assert set(attn.rotary_emb._compiled_apply) == {2, 3}
    else:
...
# ---------------------------------------------------------------------------
# Scaling-aware fused kernel: registry contract + numerical parity
# ---------------------------------------------------------------------------


def _random_qk(seed: int = 0, length: int = 8, head_dim: int = 128):
    rng = np.random.default_rng(seed)
    q = rng.standard_normal((1, 2, length, head_dim)).astype(np.float32)
    k = rng.standard_normal((1, 2, length, head_dim)).astype(np.float32)
    return mx.array(q), mx.array(k)


def _positions(length: int, ndim: int, start: int = 0):
    base = mx.arange(start, start + length, dtype=mx.int32)[None, :]
    if ndim == 2:
        return base
    # Distinct per-axis positions so a wrong axis read cannot hide.
    return mx.stack([base, base + 7, base * 2])
...
def _eager_twin(emb):
    """Same yarn state as *emb*, forced onto the eager cos/sin path."""
    twin = _bare_rotary()
    twin._inv_freq = emb.inv_freq
    twin.attention_scaling = emb.attention_scaling
    twin.fused_apply = False
    twin._compiled_apply = None
    twin.eval_cached_arrays()
    return twin


@pytest.mark.skipif(
    not mx.metal.is_available(), reason="fused mrope kernel requires Metal"
)
def test_scaled_fused_registry_installed_and_covers_both_ndims():
    emb = _yarn_rotary()
    assert emb.fused_apply is True
    registry = emb._compiled_apply
    assert registry is not None, "registry must replace the eager fallback"
    assert set(registry) == {2, 3}
...
def test_scaled_fused_kernel_matches_eager(ndim):
    emb = _yarn_rotary()
    registry = emb._compiled_apply
    assert registry is not None, "registry must replace the eager fallback"
    q, k = _random_qk()
    positions = _positions(q.shape[2], ndim)
    q_fused, k_fused = registry[ndim](
        q, k, positions, emb.inv_freq, emb.position_selector
    )
    q_eager, k_eager = _eager_twin(emb).apply_rotary(q, k, positions)
    np.testing.assert_allclose(
        np.asarray(q_fused.tolist()),
        np.asarray(q_eager.tolist()),
        rtol=1e-5,
        atol=1e-6,
    )
    np.testing.assert_allclose(
        np.asarray(k_fused.tolist()),
        np.asarray(k_eager.tolist()),
        rtol=1e-5,
        atol=1e-6,
    )
    # Passthrough lanes stay bit-exact copies through the kernel too.
    assert mx.array_equal(q_fused[..., ROTARY_DIM:], q[..., ROTARY_DIM:])
    assert mx.array_equal(k_fused[..., ROTARY_DIM:], k[..., ROTARY_DIM:])


@pytest.mark.skipif(
    not mx.metal.is_available(), reason="fused mrope kernel requires Metal"
)
def test_scaled_fused_kernel_applies_mscale():
    """The registry must bind mscale — an unscaled kernel is ~7% off."""
    emb = _yarn_rotary()
    plain = _bare_rotary()
    plain._inv_freq = emb.inv_freq  # same corrected table, scaling == 1.0
    plain.eval_cached_arrays()
    q, k = _random_qk()
    positions = _positions(q.shape[2], 2)
    q_scaled, _ = emb.apply_rotary(q, k, positions)
    q_plain, _ = plain.apply_rotary(q, k, positions)
    assert not mx.allclose(q_scaled, q_plain, rtol=1e-3, atol=1e-3).item()


def test_out_of_envelope_module_keeps_eager_fallback():
    """Even/odd pairing is outside the derived kernel's envelope."""
    ...
    assert yarn_rope.maybe_apply_yarn(emb, QWEN_512K_RECIPE) is True
    assert emb.attention_scaling != 1.0
    # The module must fall back to eager, never install the half-split
    # registry (silently wrong rotations would be worse than slow ones).
    assert emb.fused_apply is False
    assert emb._compiled_apply is None
