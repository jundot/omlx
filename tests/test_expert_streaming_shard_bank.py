...
def _store(tmp_path, e=8):
    ...
    write_safetensors(tmp_path / "model.safetensors", {key: w})
...
def test_expert_run_reads_exact_range(tmp_path, closer):
...
def test_expert_run_rejects_out_of_range(tmp_path, closer):
    store, key, e = _store(tmp_path)
    closer(store)
...
def test_pin_counts_unique_pages(tmp_path, monkeypatch, closer):
    ...
    monkeypatch.setattr(
        reader,
        "pin_expert",
        lambda k, i: (locked_calls.append(i) or _PAGE_SIZE),
    )
