...
class ScaleDependencyTests(unittest.TestCase):
    def test_cached_bf16_q_and_k_scales_preserve_rounding_boundaries(self):
        path = (
            Path(__file__).resolve().parents[1] / "omlx/patches/qwen35_gdn_prework.py"
        )
        node = next(
            n
            for n in ast.parse(path.read_text()).body
            if isinstance(n, ast.FunctionDef) and n.name == "_qwen4_scales"
        )
        calls = []

        def array(value, dtype):
            entry = SimpleNamespace(value=value, dtype=dtype)
            calls.append(entry)
            return entry
        ...
        self.assertIs(again[0], q)
        self.assertIs(again[1], k)
