...
def test_layer_context_stall_counts_layers_not_misses():
    """Per-layer stall counters: the layer is the unit, not the miss.

    From the mihailescu2m/llama.cpp research logs: "stall is per layer, not
    per miss — several misses inside one layer cost roughly what one does.
    That single fact explains why prefetch width is cheap and why miss count
    is a poor proxy for time."

    So ``decode_misses`` and ``decode_stall_rate`` can move in opposite
    directions and the stall rate is the one that tracks wall time: a policy
    that halves misses while still missing >=1 expert per layer buys ~nothing.
    """
    import numpy as np
    ...
    # Layer 1: nothing cached -> 3 projections x 3 misses = 9 misses, but it
    # is ONE stalled layer. This is the whole point of the metric.
    ctx = _LayerLoadContext(lins, cache, mode="union", positions=8)
    for lin in lins:
        ctx.ensure(lin, [2, 5, 7])
    ctx.close()
    assert cache.stats.decode_misses == 9
    assert cache.stats.decode_layers == 1
    assert cache.stats.decode_layers_missed == 1
    assert cache.stats.decode_stall_rate() == 1.0
    # Leverage: 9 misses inside 1 stalled layer -> clustered, so widening
    # prefetch (cheap per the fork) should collapse them into one stall.
    assert cache.stats.decode_misses_per_stalled_layer() == 9.0

    # Layer 2: fully cached -> still one layer-call, zero stalled.
    for lin in lins:
        for eid in (2, 5, 7):
            cache.put(lin.bundle_key(eid), (np.zeros(4, dtype=np.float32), None, None))
    ctx2 = _LayerLoadContext(lins, cache, mode="union", positions=8)
    for lin in lins:
        ctx2.ensure(lin, [2, 5, 7])
    ctx2.close()
    assert cache.stats.decode_layers == 2
    assert cache.stats.decode_layers_missed == 1
    assert cache.stats.decode_stall_rate() == 0.5

    # close() is idempotent: a second call must not double-count the layer.
    ctx2.close()
    assert cache.stats.decode_layers == 2

    # positions=None (legacy callers) contributes nothing, like the demand
    # counters it parallels.
    ctx3 = _LayerLoadContext(lins, cache, mode="union")
    for lin in lins:
        ctx3.ensure(lin, [2, 5, 7])
    ctx3.close()
    assert cache.stats.decode_layers == 2
    assert cache.stats.prefill_layers == 0

    # reset() clears the new fields too (audit P0-1: in place, never rebind).
    cache.stats.reset()
    assert cache.stats.decode_layers == 0
    assert cache.stats.decode_layers_missed == 0
...
def test_route_frequency_token_scaled_decay_window(monkeypatch):
    """Aging measured in tokens, not insertions.

    The mihailescu2m/llama.cpp logs: "Hotness decay over 1024 tokens -- at 64
    an expert accumulates ~1.5 uses between halvings, so counters cannot rank
    and eviction degenerates into plain LRU." Insertions are the wrong unit
    for us specifically because a frozen cache admits nothing (evictions 0,
    puts == size), so an insertion-counted window can sit at zero through an
    arbitrarily long decode.

    One decoded token produces ~one layer-call per MoE layer, so the window
    is tokens * num_layers in layer-calls.
    """
    from omlx.patches.expert_streaming import streaming_switch as _sw
    from omlx.patches.expert_streaming.streaming_switch import RouteFrequencyCache

    monkeypatch.setattr(_sw, "_DECAY_WINDOW_TOKENS", 4)
    rf = RouteFrequencyCache(4 * 4096, 4096, num_layers=3)
    assert rf._decay_window == 12  # 4 tokens x 3 layers

    rf.put((0, 0, "w"), ("w", "s", None))
    rf._freq[(0, 0, "w")] = 100

    # Three tokens is not a window: 3 x 3 layer-calls = 9 of 12.
    for _ in range(9):
        rf.observe_tokens(1)
    assert rf._freq[(0, 0, "w")] == 100
    assert rf.decay_diagnostics()["sweeps"] == 0

    # The 12th layer-call completes the 4-token window -> one sweep.
    for _ in range(3):
        rf.observe_tokens(1)
    assert rf._freq[(0, 0, "w")] == 50
    d = rf.decay_diagnostics()
    assert d["sweeps"] == 1
    # The degeneracy test: mean uses per counter when ranking was asked for.
    assert d["mean_uses_at_sweep"] == 100.0

    # Window 0 restores insertion-counted aging: observe_tokens is a no-op.
    monkeypatch.setattr(_sw, "_DECAY_WINDOW_TOKENS", 0)
    assert rf._decay_window == 0
    rf._freq[(0, 0, "w")] = 64
    for _ in range(100):
        rf.observe_tokens(1)
    assert rf._freq[(0, 0, "w")] == 64
    assert rf.decay_diagnostics()["sweeps"] == 1

    # clear() resets the token cursor, not just the counters.
    monkeypatch.setattr(_sw, "_DECAY_WINDOW_TOKENS", 4)
    for _ in range(3):
        rf.observe_tokens(1)
    assert rf.decay_diagnostics()["layer_calls_seen"] == 3
    rf.clear()
    assert rf.decay_diagnostics()["layer_calls_seen"] == 0


def test_decay_diagnostics_reports_both_units_unambiguously():
    """A window in tokens next to a cursor in layer-calls reads as a bug.

    The knob is expressed in tokens but ``_tokens_seen`` advances once per
    MoE layer per token, so printing the two side by side under one name
    makes a healthy 48-layer cache look like it is 48x past its window and
    never sweeping. The diagnostics carry both, and the distance to the
    next sweep in tokens.
    """
    import omlx.patches.expert_streaming.streaming_switch as _sw
    from omlx.patches.expert_streaming.streaming_switch import RouteFrequencyCache

    monkeypatch = pytest.MonkeyPatch()
    try:
        monkeypatch.setattr(_sw, "_DECAY_WINDOW_TOKENS", 1024)
        rf = RouteFrequencyCache(4096, 4096, num_layers=48)
        d = rf.decay_diagnostics()
        assert d["window_layer_calls"] == 1024 * 48
        assert d["window_tokens"] == 1024
        assert d["num_layers"] == 48
        # A 46-token decode: 46 x 48 layer-calls, far short of the window.
        for _ in range(46 * 48):
            rf.observe_tokens(1)
        d = rf.decay_diagnostics()
        assert d["sweeps"] == 0
        assert d["layer_calls_seen"] == 46 * 48
        assert d["tokens_est"] == 46.0
        assert d["tokens_to_next_sweep"] == 1024 - 46
    finally:
        monkeypatch.undo()
...
def test_adaptive_topk_configure_semantics():
    ...
    # P0-3: out-of-range must fail CLOSED to exact routing, never raise --
    # the converter's callers swallow exceptions and would otherwise leave
    # the model unconverted (resident) on a bad knob value.
    for bad in (0.01, 2.0, 1.5, -1.0, float("nan"), float("inf"), "0.9x", object()):
        adaptive_topk.configure(bad)
        assert adaptive_topk.current_threshold() is None, bad
    adaptive_topk.configure(None)
...
def test_boundary_active_for_layers_is_per_model(monkeypatch):
    """The guard verdict must reflect THIS model's layers, not the process.

    ``boundary_active()`` is process-global (true whenever any qwen class
    got wrapped by an earlier conversion); publishing it as one model's
    verdict over-credits models with no boundary and mis-attributes the
    GLM inline boundary. ``boundary_active_for_layers`` is the per-model
    truth the converter publishes.
    """
    import omlx.patches.expert_streaming.qwen35_stream_eval as qse

    InlineL = type("InlineL", (), {qse._INLINE_STREAM_EVAL_ATTR: True})
    PlainL = type("PlainL", (), {})
    # No wrapped classes in this process — the inline marker alone decides.
    monkeypatch.setattr(qse, "_candidate_decoder_classes", lambda: [])
    qse.configure_from_settings(True)
    try:
        assert qse.boundary_active_for_layers([InlineL(), InlineL()]) is True
        assert qse.boundary_active_for_layers([PlainL()]) is False
        assert qse.boundary_active_for_layers([]) is False
        # A wrapped decoder class credits only its own instances.
        WrappedL = type("WrappedL", (), {})
        setattr(WrappedL, qse._APPLIED_FLAG, True)
        monkeypatch.setattr(
            qse, "_candidate_decoder_classes", lambda: [("WrappedL", WrappedL)]
        )
        assert qse.boundary_active_for_layers([WrappedL()]) is True
        assert qse.boundary_active_for_layers([PlainL()]) is False
        # Mixed stacks (MoE + dense layers) still count when any layer runs
        # the boundary.
        assert qse.boundary_active_for_layers([PlainL(), InlineL()]) is True
    finally:
        qse.configure_from_settings(True)
    # Knob off disables every boundary, including inline ones.
    qse.configure_from_settings(False)
    try:
        assert qse.boundary_active_for_layers([InlineL()]) is False
    finally:
        qse.configure_from_settings(True)


def test_streaming_bank_bytes_boundary_relaxed_by_default():
    """A live boundary collapses the charge to the rolling live set.

    GLM-5.3 benchmark shape (42 MoE layers, pp1024): one bank per layer is
    ~30 GB of phantom charge; the relaxed charge is ~1.5 GB. Default ON."""
    import os

    from omlx.scheduler import Scheduler

    if "OMLX_STREAMING_BANK_BOUNDARY_ACCOUNT" not in os.environ:
        assert Scheduler._STREAMING_BANK_BOUNDARY_ACCOUNT is True
    back = _GuardBackingStub(42, 288, 7_100_000)
    back.streaming_guard_info.update(
        {
    ...
    model._expert_streaming_backing = back
    sched = _guard_scheduler(model)
    sched._STREAMING_BANK_BOUNDARY_ACCOUNT = True
    n = 1023
    uniq = min(288, int(sched._STREAMING_BANK_TOKEN_RATIO * n))
    tile = uniq * 7_100_000
    assert sched._streaming_bank_bytes(n) == 2 * tile + 8192 * n
    # ...and it saturates at experts_per_layer instead of growing with n.
    big = sched._streaming_bank_bytes(100_000)
    assert big == 2 * 288 * 7_100_000 + 8192 * 100_000


def test_streaming_bank_bytes_conservative_escape_hatch():
    """=0 restores the always-conservative per-layer charge (opt-out)."""
    back = _GuardBackingStub(42, 288, 7_100_000)
    back.streaming_guard_info.update(
        {
            "boundary_active": True,
    ...
    model._expert_streaming_backing = back
    sched = _guard_scheduler(model)
    sched._STREAMING_BANK_BOUNDARY_ACCOUNT = False
    n = 1023
    uniq = min(288, int(sched._STREAMING_BANK_TOKEN_RATIO * n))
    assert sched._streaming_bank_bytes(n) == 42 * uniq * 7_100_000


def test_cache_resident_bytes_counts_slots_all_policies():
    """resident_bytes = live slots x per-slot bytes on every policy."""
    from omlx.patches.expert_streaming.streaming_switch import (
        ExpertLRUCache,
        S3FIFOExpertCache,
    )

    for cls in (ExpertLRUCache, S3FIFOExpertCache):
        # num_layers=None: no per-layer caps, so all puts stay resident.
        cache = cls(100 * 1024**2, 7_100_000, None)
        assert cache.resident_bytes() == 0
        for i in range(3):
            key = (0, i, "w1")
            cache.put(key, object())
            # S3FIFO ghosts unreferenced small-queue entries; a re-reference
            # promotes them to the resident main queue (scan resistance).
            cache.get(key)
        assert cache.resident_bytes() == 3 * 7_100_000


def _lru_scheduler(heap_bytes, *, with_cache=True):
    """Guard scheduler whose backing exposes a fake LRU heap probe."""
    from types import SimpleNamespace

    back = _GuardBackingStub(42, 288, 7_100_000)
    if with_cache:
        back._streaming_cache = SimpleNamespace(
            resident_bytes=lambda: heap_bytes["now"]
        )
    model = MagicMock()
    model._expert_streaming_backing = back
    sched = _guard_scheduler(model)
    # _guard_scheduler bypasses __init__; restore the instance attr the
    # record path reads (Scheduler default is 256).
    sched._prefill_min_chunk_tokens = 32
    sched._resolve_streaming_guard_info()
    return sched


def test_record_chunk_transient_excludes_lru_fill():
    """A budget-bounded LRU fill is not linearized as transient rate."""
    from omlx.prefill_transient_tracker import PrefillTransientTracker

    heap = {"now": 100_000}
    sched = _lru_scheduler(heap)
    tracker = PrefillTransientTracker()
    sched._prefill_transient_tracker = tracker
    # Warmup chunk: 600kB footprint growth, 500kB of which is LRU fill.
    heap["now"] = 600_000
    sched._record_chunk_transient(
        100, 1_000_000, 1_600_000, request_id="r", loop_label="t"
    )
    assert tracker.samples == 1
    assert tracker.last_delta_bytes == 100_000
    assert tracker.bytes_per_token == pytest.approx(1000.0)


def test_record_chunk_transient_raw_without_cache_probe():
    """No stashable cache: raw deltas flow through (old behavior)."""
    from omlx.prefill_transient_tracker import PrefillTransientTracker

    sched = _lru_scheduler({"now": 0}, with_cache=False)
    tracker = PrefillTransientTracker()
    sched._prefill_transient_tracker = tracker
    sched._record_chunk_transient(
        100, 1_000_000, 1_600_000, request_id="r", loop_label="t"
    )
    assert tracker.last_delta_bytes == 600_000


def test_lru_shrink_never_credits_tracker():
    """Eviction shrink reports 0 growth, never a negative credit."""
    from omlx.prefill_transient_tracker import PrefillTransientTracker
    ...
    tracker = PrefillTransientTracker()
    sched._prefill_transient_tracker = tracker
    heap["now"] = 100_000  # governor pressure evicted entries
    sched._record_chunk_transient(
        100, 1_000_000, 1_600_000, request_id="r", loop_label="t"
    )
    assert tracker.last_delta_bytes == 600_000
...
def test_read_expert_banks_no_bridge_without_split():
    """Fase K K5 + re-gate: the C2 bank reader returns exactly the demanded
    ids — no hole rows, ever. Bridging used to be gated on the HOBBIT split
    and was removed outright (audit 2026-09-09, P2-13), so this now holds
...
class TestFaseKRunMerge:
    """Fase K F7 run grouping.

    Gap bridging was REMOVED (audit 2026-09-09, P2-13): it measured a net
    loss in both regimes, so `_group_runs` never bridges any more. The
    primitive still lives in `shard_bank.segment_runs(merge_gap=...)` and is
    covered by test_read_expert_into_bridges_gap_but_scatters_only_demand.
    These tests pin what remains: plain contiguous grouping, tier boundaries
    that never merge, and `max_run` still bounding each run.
    """

    def test_group_runs_never_bridges_a_gap(self):
        """P2-13: a hole in the demand set is never absorbed into one run,
        with or without the HOBBIT split active."""
        from omlx.patches.expert_streaming import streaming_switch as ss

        assert not hasattr(ss, "_RUN_MERGE_GAP"), "the removed knob came back"

        lin = _make_advise_linear(0, "gate_proj", _AdviseRecorderBacking(), ss.ExpertLRUCache(0, 4096))
        # No split: contiguous runs only.
        assert lin._group_runs([3, 4, 7, 8]) == [(3, 2), (7, 2)]
        assert lin._group_runs([3, 4, 8, 9]) == [(3, 2), (8, 2)]
        assert lin._group_runs([3, 4, 5]) == [(3, 3)]
        # Split active: still no bridging (this is the regression guard —
        # the old code produced [(3, 6)] here when the env was set).
        lin.set_hobbit_split({3, 4, 5, 6, 7, 8, 9}, cold_bits=2, cold_gs=32)
        assert lin._group_runs([3, 4, 7, 8]) == [(3, 2), (7, 2)]
        assert lin._group_runs([3, 4, 8, 9]) == [(3, 2), (8, 2)]

    def test_group_runs_max_run_bounds_each_run(self):
        """max_run still splits a contiguous run at the cap."""
        from omlx.patches.expert_streaming import streaming_switch as ss

        lin = _make_advise_linear(0, "gate_proj", _AdviseRecorderBacking(), ss.ExpertLRUCache(0, 4096))
        with patch.object(ss, "_RUN_MAX", 4):
            assert lin._group_runs([3, 4, 5, 6, 7, 8]) == [(3, 4), (7, 2)]
        with patch.object(ss, "_RUN_MAX", 2):
            assert lin._group_runs([3, 4, 5]) == [(3, 2), (5, 1)]

    def test_group_runs_never_crosses_tier(self):
        """Under the HOBBIT split a run stays inside one tier — hot ids and
        cold ids never share a coalesced pread."""
        from omlx.patches.expert_streaming import streaming_switch as ss
...
# ---------------------------------------------------------------------------
# Audit 2026-09-09 remediation (docs/expert-streaming-audit-2026-09-09.md)
# ---------------------------------------------------------------------------


def test_cache_get_put_survive_concurrent_threads():
    """P0-1: the LRU is touched by the MLX thread, warm-pool workers and the
    asyncio loop. Every mutation now runs under one RLock — hammer it from
    every side at once and assert the invariants that used to break."""
    from omlx.patches.expert_streaming.streaming_switch import make_expert_cache

    for policy in ("lru", "s3fifo", "route_frequency"):
        cache = make_expert_cache(1 << 22, 4096, num_layers=4, policy=policy)
        ...
        def hammer(tid: int) -> None:
            try:
                ...
                while not stop.is_set() and n < 400:
                    key = (n % 4, (n * 7 + tid) % 32, "gate_proj")
                    if cache.get(key) is None:
                        cache.put(key, ("v", tid, n))
                    if n % 50 == 0:
                        cache.retain_hot({(0, 1), (1, 2)})
                    n += 1
        ...
        def resizer() -> None:
            try:
                ...
                while not stop.is_set() and n < 200:
                    cache.resize(max(1, cache.capacity // 2), 1)
                    cache.resize(cache.capacity * 2, 1)
                    n += 1
        ...
        for t in threads:
            t.join(timeout=30)
        stop.set()
        assert not errors, f"{policy}: {errors!r}"
        assert not any(t.is_alive() for t in threads), f"{policy}: thread hung"
        # Invariants: never over capacity, and per-layer counts agree with
        # the store (the old racy read-modify-write desynced them).
        assert cache.size <= max(1, cache.capacity)
        total = sum(cache._layer_counts.values())
        assert total == cache.size, f"{policy}: counts {total} != size {cache.size}"


def test_cache_clear_resets_stats_in_place():
    """P0-1: clear() used to REBIND stats, orphaning the object
    _count_demand() holds while it increments."""
    from omlx.patches.expert_streaming.streaming_switch import ExpertLRUCache

    cache = ExpertLRUCache(1 << 20, 1024, num_layers=2)
    cache.put((0, 0, "gate_proj"), "v")
    held = cache.stats
    held.hits = 7
    held.evictions = 3
    cache.clear()
    assert cache.stats is held, "stats identity must survive clear()"
    assert held.hits == 0 and held.evictions == 0


def test_governor_resize_drains_s3fifo_queues():
    """P0-2: the governor only drained _store, so an S3-FIFO cache kept its
    small/ghost queues and never actually shrank under memory pressure."""
    from omlx.patches.expert_streaming.streaming_switch import make_expert_cache

    cache = make_expert_cache(1 << 24, 4096, num_layers=1, policy="s3fifo")
    for i in range(64):
        cache.put((0, i, "gate_proj"), f"v{i}")
    filled = cache.size
    assert filled > 8, "fixture should fill both queues"
    assert cache._small, "expected entries in the small FIFO"
    cache.resize(4, 4)
    assert cache.size <= 4, f"s3fifo did not shrink: {cache.size}"
    assert cache.capacity == 4


def test_governor_reconciles_per_slot_after_layout_reconciliation():
    """P1-5: a fused gate_up model reconciles per_slot from per_expert//3 to
    per_expert//2. The governor kept the stale number and grew the cache
    ~1.5x past the user budget."""
    from omlx.patches.expert_streaming.governor import ExpertResidencyGovernor

    cache = MagicMock()
    cache.capacity = 100
    gov = ExpertResidencyGovernor(cache, per_slot=1000, num_layers=4, max_budget_bytes=1 << 30)
    assert gov.per_slot == 1000
    cap, _ = gov._caps_for(1 << 20)
    assert cap == (1 << 20) // 1000
    gov.reconcile_per_slot(1500)
    assert gov.per_slot == 1500
    cap, _ = gov._caps_for(1 << 20)
    assert cap == (1 << 20) // 1500
    # Garbage must be ignored, not crash the governor mid-request.
    gov.reconcile_per_slot("nope")
    assert gov.per_slot == 1500


def test_governor_apply_prefers_atomic_cache_resize():
    """P0-2: _apply must delegate to cache.resize() when available so the
    whole retarget is one atomic step under the cache lock."""
    from omlx.patches.expert_streaming.governor import ExpertResidencyGovernor

    cache = MagicMock()
    cache.capacity = 64
    cache.resize = MagicMock()
    gov = ExpertResidencyGovernor(cache, per_slot=4096, num_layers=2, max_budget_bytes=1 << 30)
    gov._apply(16, 8)
    cache.resize.assert_called_once_with(16, 8)
    # The field-by-field fallback stays for duck-typed caches without resize.
    plain = MagicMock(spec=["capacity", "_store", "_layer_counts", "stats"])
    plain.capacity = 64
    plain._store = {}
    plain._layer_counts = {}
    gov2 = ExpertResidencyGovernor(plain, per_slot=4096, num_layers=0, max_budget_bytes=1 << 30)
    gov2._apply(16, 8)
    assert plain.capacity == 16


def test_governor_dynamic_knobs_read_env(monkeypatch):
    """P2-14: the opt-in and its 6 GiB ceiling were untested."""
    from omlx.patches.expert_streaming import governor as gov_mod

    monkeypatch.setattr(gov_mod, "_DYNAMIC_ENV", False)
    assert gov_mod.dynamic_residency_enabled() is False
    monkeypatch.setattr(gov_mod, "_DYNAMIC_ENV", True)
    assert gov_mod.dynamic_residency_enabled() is True

    monkeypatch.delenv("OMLX_EXPERT_STREAMING_DYNAMIC_MAX_GIB", raising=False)
    assert gov_mod._max_dynamic_budget_bytes() == int(6.0 * 1024**3)
    monkeypatch.setenv("OMLX_EXPERT_STREAMING_DYNAMIC_MAX_GIB", "2.5")
    assert gov_mod._max_dynamic_budget_bytes() == int(2.5 * 1024**3)
    monkeypatch.setenv("OMLX_EXPERT_STREAMING_DYNAMIC_MAX_GIB", "garbage")
    assert gov_mod._max_dynamic_budget_bytes() == int(6.0 * 1024**3)
    monkeypatch.setenv("OMLX_EXPERT_STREAMING_DYNAMIC_MAX_GIB", "-4")
    assert gov_mod._max_dynamic_budget_bytes() == 0


def test_hot_fraction_one_is_all_hot_not_all_cold():
    """P1-9: hf == 1.0 used to fall through to the fully-cold scale — the
    opposite of the runtime, which elects ceil(1.0 * width) hot experts.

    The guard was ``0.0 < hf < 1.0``, so the "everything is hot" setting
    charged the *cold* per-expert bytes. It is now ``hf > 0.0``.
    """
    from omlx.patches.expert_streaming import residency as res
    ...
    with tempfile.TemporaryDirectory() as td:
        ...
        model.mkdir()
        key = "model.layers.0.mlp.switch_mlp.gate_proj.weight"
        # Source: 4 * 16 * 32 * 2B = 4096 expert bytes.
        _write_shard(model / "model.safetensors", {key: ((4, 16, 32), "BF16")})
        (model / "config.json").write_text(
            json.dumps(
                {
                    "model_type": "qwen3_moe",
        ...
        base = res._cached_estimate(
            str(model), res._sig_of(model), res._index_sig_of(model)
        )
        assert base.supported, f"fixture not supported: {base.reason}"
        assert base.expert_bytes == 4096, base.expert_bytes

        # Cold tier repacks the same expert into a quarter of the bytes.
        cold = model / "expert_cold"
        cold.mkdir()
        _write_shard(cold / "cold.safetensors", {key: ((4, 16, 8), "BF16")})

        scale = 0.25  # 1024 / 4096
        # No hot fraction -> the measured cold scale, uniform tier.
        assert res._resolve_cold_overlay(model, cold_root=cold) == (scale, "uniform")
        assert res._resolve_cold_overlay(model, cold_root=cold, hot_fraction=0.0) == (
            scale,
            "uniform",
        )
        # P1-9: hf == 1.0 means EVERY expert is stored at full precision, so
        # decode reads the SOURCE bytes -> scale 1.0, not 0.25.
        got = res._resolve_cold_overlay(model, cold_root=cold, hot_fraction=1.0)
        assert got == (1.0, "hobbit"), got
        # A genuine blend still lands between the two.
        got = res._resolve_cold_overlay(model, cold_root=cold, hot_fraction=0.5)
        assert got is not None and abs(got[0] - (0.5 + 0.5 * scale)) < 1e-9, got
        assert got[1] == "hobbit"
        # Junk hot fractions must not raise out of the admission path.
        assert res._resolve_cold_overlay(
            model, cold_root=cold, hot_fraction="not-a-number"
        ) == (scale, "uniform")


def test_read_failures_counter_is_locked():
    """P2-11: the per-key failure counter was a bare dict RMW from IO
    workers — concurrent bumps on one key lost increments."""
    from omlx.patches.expert_streaming.streaming_switch import ExpertLRUCache

    cache = ExpertLRUCache(1 << 20, 1024, num_layers=1)
    key = (0, 3, "gate_proj")
    errors: list = []
    ...
    for t in threads:
        t.join(timeout=30)
    assert not errors
    assert cache.read_failure_stats()[key] == 800


def test_pin_bookkeeping_released_on_unpin_and_close(monkeypatch):
    """P1-6: pins were accumulated and never munlock'd; close() left
    pinned_bytes reporting wired memory that no longer existed."""
    from omlx.patches.expert_streaming import shard_bank as sb

    monkeypatch.setattr(sb, "_mlock_range", lambda *a, **k: True)
    unlocked: list = []
    monkeypatch.setattr(sb, "_munlock_range", lambda *a, **k: (unlocked.append(a[1:]), True)[1])

    with tempfile.TemporaryDirectory() as td:
        root = Path(td)
        model = root / "model"
        model.mkdir()
        _write_shard(model / "model.safetensors", {"model.layers.0.mlp.switch_mlp.gate_proj.weight": ((4, 16, 32), "BF16")})
        (model / "model.safetensors.index.json").write_text(
            json.dumps({"weight_map": {"model.layers.0.mlp.switch_mlp.gate_proj.weight": "model.safetensors"}})
        )
        backing = sb.ExpertBackingStore(model)
        key = "model.layers.0.mlp.switch_mlp.gate_proj.weight"
        assert backing.pin_expert(key, 0) > 0
        assert backing.pinned_count == 1
        assert backing.pinned_bytes > 0
        # Duplicate pin is skipped.
        assert backing.pin_expert(key, 0) == 0
        assert backing.pinned_count == 1
        assert backing.unpin_all() == 1
        assert backing.pinned_count == 0
        assert backing.pinned_bytes == 0
        assert len(unlocked) == 1

        # close() must release pins too, not just close the readers.
        backing.pin_expert(key, 1)
        assert backing.pinned_count == 1
        backing.close()
        assert backing.pinned_count == 0
        assert backing.pinned_bytes == 0
...
def test_reader_cache_publishes_one_canonical_instance():
    """P1-7: two threads racing a lazy open used to publish two readers for
    one path, leaking the loser's fd + mmap and breaking the tier contract
    ("if any(r is not reader): return False") in read_expert_into."""
    from omlx.patches.expert_streaming.shard_bank import ExpertBackingStore
    ...
    with tempfile.TemporaryDirectory() as td:
        ...
        backing = ExpertBackingStore(model)
        seen: list = []
        lock = threading.Lock()
        ...
        def resolve() -> None:
            start.wait()
            r = backing._reader_for_key_source(key)
            with lock:
                seen.append(r)
        ...
        for t in threads:
            t.join(timeout=30)
        assert len(seen) == 8
        assert len({id(r) for r in seen}) == 1, "readers diverged under concurrency"
        assert len(backing._readers) == 1, "duplicate readers left in the cache"
        backing.close()


def test_estimate_cache_invalidates_on_checkpoint_change():
    """P2-14: the whole invalidation path (_cached_estimate / _sig_of /
    _index_sig_of) was untested — a stale signature silently served a
    pre-repackaging estimate."""
    from omlx.patches.expert_streaming import residency as res
    ...
    with tempfile.TemporaryDirectory() as td:
        ...
        _write_shard(model / "model.safetensors", {
            "model.layers.0.mlp.switch_mlp.gate_proj.weight": ((4, 16, 32), "BF16"),
        })
        cfg = {"model_type": "qwen3_moe", "num_hidden_layers": 1, "hidden_size": 32,
               "moe_intermediate_size": 16, "num_experts": 4, "num_experts_per_tok": 1}
        (model / "config.json").write_text(json.dumps(cfg))
        est1 = res.expert_streaming_estimate(model)
        hits1 = res._cached_estimate.cache_info().hits
        res.expert_streaming_estimate(model)
        assert res._cached_estimate.cache_info().hits > hits1, "second scan should hit"
        # A new shard changes the signature -> must miss.
        _write_shard(model / "model2.safetensors", {
            "model.layers.1.mlp.switch_mlp.gate_proj.weight": ((4, 16, 32), "BF16"),
        })
        res.expert_streaming_estimate(model)
        assert est1.num_moe_layers >= 1


def test_budget_clamped_in_loader_path():
    """P2-16: the PUT handler clamps 0-64 GiB; the loader path had no clamp,
    so a hand-edited settings file could ask for a heap past physical RAM."""
    from omlx.patches.expert_streaming import (
        MAX_EXPERT_STREAMING_BUDGET_BYTES,
        _get_budget_bytes,
    )

    settings = ModelSettings(expert_streaming_budget_gib=500.0)
    assert _get_budget_bytes(settings, None) == MAX_EXPERT_STREAMING_BUDGET_BYTES
    settings = ModelSettings(expert_streaming_budget_gib=-3.0)
    assert _get_budget_bytes(settings, None) == 0
    settings = ModelSettings(expert_streaming_budget_gib=2.0)
    assert _get_budget_bytes(settings, None) == 2 * 1024**3


@pytest.mark.slow
def test_streaming_decode_tokens_match_resident():
    """P0-4: the public bit-exactness claim is about TOKENS.

    The repo had tensor/kernel-level gates (test_streaming_glu_matches_reference
    and friends) but nothing that compared generated token IDs, and the only
    token gate lived in bench/bench_expert_streaming.py --gate-tokens, run by
    hand. This is the automated version: a tiny MoE LM is run greedy-decoded
    twice, once through mlx-lm's resident SwitchGLU and once through the
    streaming replacement, with a router that reads the previous layer's
    output -- so any divergence in the weights propagates into the routing
    and then into the token stream.

    It is marked slow (excluded from the default run) because it drives a
    real forward pass per decode step.
    """
    import mlx.core as mx
    ...
    E, H, D, V, L = 8, 32, 16, 24, 2
    rng = np.random.default_rng(7)
    mx.random.seed(7)

    ref_layers = [SwitchGLU(D, H, E) for _ in range(L)]
    mx.eval([p.parameters() for p in ref_layers])

    cache = ExpertLRUCache(1 << 26, 1 << 18, num_layers=L)
    backing: dict = {}
    str_layers = []
    for li, ref in enumerate(ref_layers):
        glu = StreamingSwitchGLU(
            input_dims=D, hidden_dims=H, num_experts=E, layer_idx=li,
            backing=backing, cache=cache, quantized=False, activation=None,
        )
        for name in ("gate_proj", "up_proj", "down_proj"):
            ref_lin = getattr(ref, name)
            backing[(li, name)] = ref_lin.weight
            setattr(glu, name, StreamingSwitchLinear(
                layer_idx=li, proj_name=name, stacked_key=f"k{li}.{name}",
                num_experts=E, input_dims=ref_lin.input_dims,
                output_dims=ref_lin.output_dims, backing=backing, cache=cache,
            ))
        str_layers.append(glu)

    emb = mx.array(rng.standard_normal((V, D)).astype(np.float32) * 0.5)
    routers = [mx.array(rng.standard_normal((D, E)).astype(np.float32) * 0.5) for _ in range(L)]
    head = mx.array(rng.standard_normal((D, V)).astype(np.float32) * 0.5)
    mx.eval(emb, *routers, head)

    def generate(layers, prompt, steps):
        toks = list(prompt)
        for _ in range(steps):
            x = emb[mx.array(toks)][None]  # (1, T, D)
            for li, glu in enumerate(layers):
                idx = mx.argmax(x @ routers[li], axis=-1, keepdims=True).astype(mx.int32)
                x = glu(x, idx)[:, :, 0, :]
            mx.eval(x)
            logits = x[:, -1, :] @ head
            toks.append(int(mx.argmax(logits, axis=-1).item()))
        return toks

    prompt = [1, 5, 9]
    resident = generate(ref_layers, prompt, 12)
    streaming = generate(str_layers, prompt, 12)
    assert streaming == resident, (
        "streaming decode diverged from resident: "
        f"{streaming} != {resident}"
    )
    # A gate that passes because nothing was generated is worthless.
    assert len(set(resident)) > 2, f"degenerate token stream: {resident}"
    assert cache.stats.misses > 0, "streaming path never read through the backing"


def test_qwen4_admission_ceiling_is_shared_by_admission_and_loader():
    """P1-10: admission priced the Qwen4-Exp PLE call against a live ceiling
    while the loader passed None — and `_qwen4_ple_offload_status` picks a
    *different* ceiling for None (the stable residency one). A model could
    therefore be admitted by its mmap size and then loaded resident.

    Both sites now go through one helper, so a single ceiling is observable
    and testable.
    """
    pool = EnginePool()
    entry = EngineEntry(
    ...
    # Non-Qwen4 models have no PLE ceiling call to make.
    other = EngineEntry(
        model_id="glm", model_path="/tmp/glm", model_type="llm",
        engine_type="batched", estimated_size=1,
    )
    other.config_model_type = "glm_moe_dsa"
    assert pool._qwen4_admission_ceiling(other) is None

    # No wired ceiling -> None (callers fall back to their own default).
    assert pool._qwen4_admission_ceiling(entry) is None

    # A live ceiling is forwarded verbatim.
    pool._get_final_ceiling = lambda: 48 * 1024**3
    assert pool._qwen4_admission_ceiling(entry) == 48 * 1024**3

    # Zero live ceiling falls back to the static admission ceiling rather
    # than silently diverging to "None" (which meant a different number).
    pool._get_final_ceiling = lambda: 0
    pool._get_admission_ceiling = lambda: 32 * 1024**3
    assert pool._qwen4_admission_ceiling(entry) == 32 * 1024**3


def test_shard_reader_pin_expert_direct(monkeypatch):
    """P2-14: `_ShardReader.pin_expert` (shard_bank.py:1012) had no direct
    test — only the `ExpertBackingStore` wrapper around it was exercised, so
    the page-rounding arithmetic and the mlock-refused path were uncovered.

    The wrapper's dedupe and cross-expert page accounting are pinned by
    test_pin_bookkeeping_released_on_unpin_and_close; this one pins the
    reader's own contract, page-size agnostic (16 KB pages on Apple
    Silicon, 4 KB elsewhere):

    * it hands _mlock_range the EXACT expert slice (offset, length);
    * it reports the WIRED bytes — page-rounded, so >= the slice, never less
      (under-reporting would let pin_gib overshoot the real wired cost);
    * mlock refusing yields 0, not a raise and not a phantom byte count;
    * unpin mirrors the identical range back to _munlock_range.
    """
    from omlx.patches.expert_streaming import shard_bank as sb
    ...
    locked: list = []
    unlocked: list = []
    monkeypatch.setattr(sb, "_mlock_range", lambda mm, off, ln: (locked.append((off, ln)), True)[1])
    monkeypatch.setattr(
        sb, "_munlock_range", lambda mm, off, ln: (unlocked.append((off, ln)), True)[1]
    )

    def wired(off: int, end: int, mmap_len: int) -> int:
        start_pg = (off // P) * P
        end_pg = min(mmap_len, ((end + P - 1) // P) * P)
        return end_pg - start_pg

    with tempfile.TemporaryDirectory() as td:
        shard = Path(td) / "model.safetensors"
        key = "model.layers.0.mlp.switch_mlp.gate_proj.weight"
        # (4, 16, 32) BF16: a 4-expert bank, 1024 B per expert.
        _write_shard(shard, {key: ((4, 16, 32), "BF16")})
        reader = sb._ShardReader(shard)
        try:
            mlen = len(reader._mmap)
            for eid in (0, 2, 3):
                off, end = reader.expert_byte_range(key, eid)
                assert end - off == 1024, (eid, off, end)
                n = reader.pin_expert(key, eid)
                # Exact slice handed to mlock...
                assert locked[-1] == (off, 1024), locked[-1]
                # ...and page-rounded, never-under-reported wired bytes.
                assert n == wired(off, end, mlen), (eid, n)
                assert n >= 1024, (eid, n)
                assert n % P == 0 or end + P > mlen, (eid, n)

            # Two experts inside one page report the SAME wired bytes — the
            # wrapper's _pinned_pages is what stops that from double-counting.
            assert reader.pin_expert(key, 0) == reader.pin_expert(key, 1)

            # mlock refusing: 0 bytes, no raise, nothing reported.
            monkeypatch.setattr(sb, "_mlock_range", lambda *a, **k: False)
            assert reader.pin_expert(key, 1) == 0

            # unpin mirrors the same range back.
            off, end = reader.expert_byte_range(key, 3)
            assert reader.unpin_expert(key, 3) == wired(off, end, mlen)
            assert unlocked[-1] == (off, 1024), unlocked[-1]
        finally:
            reader.close()


def test_page_cache_probe_mincore_units(tmp_path):
    """mincore probe: real residency, and `None` is never "cold".

    The point of the probe is to replace a FITTED page-cache split
    (1/10.55 = 0.75/82 + 0.25/D implied 75% of expert bytes came from the
    page cache) with a direct counter, because the mihailescu2m/llama.cpp
    logs measured the opposite -- "the page cache serves a few percent of
    expert reads" for multi-MiB slabs. A probe that cannot answer must say
    so rather than silently vote for one side.
    """
    import mmap

    from omlx.patches.expert_streaming import shard_bank as sb

    # Unanswerable cases return None -- callers treat it as unknown.
    assert sb._mincore_resident_bytes(None, 0, 4096) is None
    f = tmp_path / "f.bin"
    f.write_bytes(b"\0" * (64 * 1024))
    with open(f, "rb") as fh:
        mm = mmap.mmap(fh.fileno(), 0, access=mmap.ACCESS_READ)
        try:
            assert sb._mincore_resident_bytes(mm, 0, 0) is None
            # Touch the first 32 KiB so those pages are certainly in core,
            # then probe: the touched range must report resident.
            mm[0 : 32 * 1024]
            hot = sb._mincore_resident_bytes(mm, 0, 32 * 1024)
            assert hot is not None
            assert hot == 32 * 1024
        finally:
            mm.close()

    # Counter arithmetic and reset.
    p = sb.PageCacheProbe()
    p.record(10, 90)
    p.record(0, 50)
    p.failed()
    ...
    assert s["cold_bytes"] == 140
    assert s["sampled_bytes"] == 150
    assert s["failures"] == 1
    assert abs(s["hot_byte_frac"] - 10 / 150) < 1e-12
    p.reset()
    assert p.summary()["calls"] == 0
    assert p.summary()["hot_byte_frac"] == 0.0


def test_page_cache_probe_sampled_on_every_read(monkeypatch):
    """The probe sits in _read_into, the single funnel for expert reads.

    Sampling anywhere else would miss a path and quietly bias the answer --
    which is exactly how the fitted 75% survived as long as it did.
    """
    import numpy as np

    from omlx.patches.expert_streaming import shard_bank as sb

    monkeypatch.setattr(sb, "_PCACHE_PROBE_ENV", True)
    probe = sb.PageCacheProbe()
    monkeypatch.setattr(sb, "_PCACHE_PROBE_SINGLETON", probe)
    monkeypatch.setattr(sb, "page_cache_probe", lambda: probe)
    ...
    class _Reader:
        ...
        def _read_into(self, abs_off, out):
            # Reuse the real hook by driving the production method directly.
            return sb._ShardReader._read_into(self, abs_off, out)

    # Drive the real code path with a stubbed mincore so the test asserts the
    # WIRING (offset/length handed over, bytes split, one sample per read)
    # rather than the machine's page-cache state.
    def fake_mincore(mm, off, length):
        calls.append((off, length))
        return min(1024, length)

    monkeypatch.setattr(sb, "_mincore_resident_bytes", fake_mincore)
    ...
    with tempfile.TemporaryDirectory() as td:
        ...
        try:
            ...
            reader._read_into(reader.expert_byte_range(key, 0)[0], buf)
            buf2 = np.empty(2048, dtype=np.uint8)
            reader._read_into(reader.expert_byte_range(key, 1)[0], buf2)
        finally:
            reader.close()

    assert len(calls) == 2
    assert calls[0][1] == 2048 and calls[1][1] == 2048
    # mincore stub returns 1024 hot of 2048 -> 50/50 per read.
    s = probe.summary()
    assert s["calls"] == 2
    assert s["hot_bytes"] == 2048
    assert s["cold_bytes"] == 2048
    assert abs(s["hot_byte_frac"] - 0.5) < 1e-12



def test_no_dedicated_prefetch_queue():
    """P2-13: the dedicated prefetch queue port was removed 2026-09-09.

    Prefetch now shares the demand pool (the knob's default behaviour), so
    the process-wide IO depth is unchanged and never silently grown.
    """
    from omlx.patches.expert_streaming import streaming_switch as _sw

    # No separate queue object.
    assert not hasattr(_sw, "prefetch_pool_for_positions")
    assert not hasattr(_sw, "_PREFETCH_IO_POOL")
    # The demand pool owns the full process-wide depth.
    assert _sw._EXPERT_IO_POOL._max_workers == _sw._IO_QD
    assert _sw._EXPERT_IO_POOL._max_workers >= 1
