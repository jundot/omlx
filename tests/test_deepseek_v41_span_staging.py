...
def test_advise_demand_filters_residents(tmp_path):
...
def test_advise_demand_noop_on_source_checkpoint(tmp_path):
...
def test_prefill_call_advises_upcoming_chunks(tmp_path):
...
def test_prefill_advises_next_layer_bank(tmp_path):
    ...
    try:
        ...
        slots_last = disk.language_model.layers[n_layers - 1].ffn.experts.slots
...
def test_ra_disabled_skips_all_advise(tmp_path, monkeypatch):
...
def test_prefill_transient_cap_widens_and_restores(tmp_path, monkeypatch):
...
def test_prefill_cap_restores_on_mid_call_failure(tmp_path, monkeypatch):
...
def test_chunk_lookahead_stages_exact_demand(tmp_path):
...
def test_chunk_lookahead_disabled(tmp_path, monkeypatch):
...
def test_verify_round_advises_next_layer_union(tmp_path):
