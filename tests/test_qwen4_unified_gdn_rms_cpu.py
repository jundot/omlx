...
SOURCE = (
    Path(__file__).parents[1] / "omlx/patches/qwen4_unified_gdn_verify.py"
).read_text()
...
class RMSCandidateContractTests(unittest.TestCase):
    def test_every_non_normalization_kernel_byte_preserved(self):
        ...
        begin = source.index(
            "    // Preserve oMLX generic RMS/scaling operation order exactly:"
        )
        end = source.index("    device float* state_dst =", begin)
        rest = source[:begin] + source[end:]
        self.assertEqual(
            hashlib.sha256(rest.encode()).hexdigest(),
            CONSTANTS["NONNORMALIZATION_SHA256"],
        )
    ...
    def _wrapper(self):
        fn = next(
            ...
            and n.name == "qwen4_unified_gdn_verify_rms"
        )
        calls = []
        ...
        namespace = dict(
            CONSTANTS, mx=SimpleNamespace(float32="float32"), _kernel=lambda: dispatch
        )
        exec(
            compile(
                ast.Module(body=[fn], type_ignores=[]), "candidate-wrapper", "exec"
            ),
            namespace,
        )
    ...
    def test_unqualified_width_and_geometry_refused_before_dispatch(self):
        ...
        for width, ty in ((2, 8), (9, 8), (3, 7)):
            with self.assertRaises(ValueError):
                fn(
                    SimpleNamespace(shape=(1, width, 10240), dtype="bfloat16"),
                    *([None] * 10),
                    threadgroup_y=ty,
                )
