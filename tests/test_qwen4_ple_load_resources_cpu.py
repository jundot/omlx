...
def wrapper():
    ...
    node = next(n for n in tree.body if isinstance(n, ast.FunctionDef) and n.name == '_force_qwen4_exp_sanitize_on_load')
    # Supply the exact resource module without importing the MLX engine/package.
    imports = [n for n in node.body if isinstance(n, ast.ImportFrom) and n.module.endswith('ple_load_resources')]
    assert len(imports) == 1 and imports[0].names[0].name == 'ple_load_resources'
    node.body.remove(imports[0])
    ns = dict(contextlib=contextlib, Path=Path, ple_load_resources=resources.ple_load_resources,
              _read_config_model_type=lambda _: 'qwen4_exp',
              _model_shard_matcher=lambda _: lambda _: False,
              logger=logging.getLogger(__name__))
    exec(compile(ast.Module(body=[node], type_ignores=[]), 'live-vlm-wrapper', 'exec'), ns)
...
class LoadResourceTests(unittest.TestCase):
    def setUp(self):
        ...
        runtime.NS['mx'] = types.SimpleNamespace(ones=lambda shape, dtype: np.ones(shape, dtype=dtype), bfloat16=np.float16)
        ...
        self.model_file.write_bytes(runtime.struct.pack('<Q', len(encoded)) + encoded + data[8+size:])
        (Path(self.fixture.temp.name) / 'model.safetensors.index.json').write_text(json.dumps({'weight_map': {'table.shard_0.weight': self.model_file.name}}))
        self.safe_open = lambda *a, **k: None
        self.modules = patch.dict(sys.modules, {'safetensors': types.SimpleNamespace(safe_open=self.safe_open)})
        self.modules.start()
    ...
    def embedding(self, shards=1):
        return runtime.NS['DiskBackedShardedEmbedding'](self.fixture.temp.name, 'table', 16 * shards, 4, shards)
    ...
    def test_concurrent_load_failure_does_not_close_other_scope(self):
        ...
        def load(name, fail):
            try:
                ...
                errors.append(exc)
        threads = [threading.Thread(target=load, args=('failure', True)), threading.Thread(target=load, args=('success', False))]
    ...
    def test_cleanup_orders_embedding_before_reader_and_preserves_error(self):
        ...
        class Owner:
            ...
            def close(self):
                ...
                if self.fails: raise OSError('cleanup error')
