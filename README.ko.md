...
- **모델 프로필**: 각 모델 프로필에 대해 사용자 정의 설정을 적용할 수 있습니다. `:` 구분자로 "실시간"으로 사용할 수 있습니다. `(model_name|model_alias):model_profile name` 형식으로 요청 시 선택된 프로필로 처리됩니다. 프로필이 제공되지 않으면 기본값을 사용합니다.
