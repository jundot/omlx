...
// Decode only the channel tile currently consumed by QK or PV.
METAL_FUNC float deepseek_v41_fp8(uchar code) {
...
METAL_FUNC float deepseek_v41_packed_value(
    const device uchar* row, int d, bool pooled) {
  if (pooled) {
    constexpr float levels[8] = {0, 0.5f, 1, 1.5f, 2, 3, 4, 6};
    const uint code = (row[d / 2] >> ((d % 2) * 4)) & 15;
    const float value = levels[code & 7] * ((code & 8) ? -1.0f : 1.0f);
    return value * deepseek_v41_fp8(row[256 + d / 16]);
  }
  return ldexp(deepseek_v41_fp8(row[d]), int(row[512 + d / 32]) - 127);
}
...
[[kernel, max_total_threads_per_threadgroup(WM * 32)]] void deepseek_v41_packed_attention(
    ...
    const device uchar* PooledKV [[buffer(2)]],
    const device IndexT* Topk [[buffer(3)]],
    const device float* Sinks [[buffer(4)]],
    device T* O [[buffer(5)]],
  ...
  static_assert(H == 64 && D == 512, "Packed attention requires 64 heads of width 512.");
  ...
  const device uchar* local_base =
      LocalKV + size_t(b) * params->Local_strides[0];
  const device uchar* pooled_base =
