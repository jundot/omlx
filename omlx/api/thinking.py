            ...
            if text[i] == '<':
                ...
                if self._could_be_tag(remaining):
                    ...
                    if track:
                        ...
                        # A later chunk may opt into logprobs after this
                        # untracked partial tag. Keep positional placeholders
                        # so the mixed path cannot index a short list or claim
                        # an alignment it does not have.
                        self._buffer_lps = [None] * len(remaining)
                    break
