...
class Scheduler:
    ...
    def _do_external_prefill(
        ...
        while input_arr.shape[1] > 0:
            ...
            with mx.stream(self._stream):
                ...
                if embeds_array is None:
                    Scheduler._prefetch_ple_chunks(
                        self, input_arr, n_to_process, processed_tokens,
                        base_size, boundary_enabled, block_size,
                    )
    ...
    def _prefetch_ple_chunks(
        self, tokens, n, processed, base_size, boundary_enabled, block_size
    ) -> None:
        """Prefetch only after the guards choose the current chunk size."""
        prefetch = getattr(self.model, "prefetch_ple", None)
        if prefetch is None:
            return
        current = tokens[:, :n]
        if processed == 0:
            prefetch(current, tokens[:, :0])
        remaining = tokens.shape[1] - n
        if remaining > 0:
            next_n = _expected_chunk_len(
                min(n, self._prefill_step_size_for_progress(processed + n, remaining)),
                remaining,
                base_size + processed + n,
                boundary_enabled,
                block_size,
            )
            prefetch(tokens[:, n : n + next_n], current)
    ...
    def _step_prefill_chunk(self, state: _PrefillState) -> bool:
        ...
        with mx.stream(self._stream):
            Scheduler._prefetch_ple_chunks(
                self, state.tokens_remaining, n, state.tokens_processed,
                state.base_size, state.boundary_enabled, state.block_size,
            )
