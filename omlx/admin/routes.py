...
async def list_models(is_admin: bool = Depends(require_admin)):
    ...
    for model_info in models_status:
        ...
        distributed = bool(model_info.get("distributed", False))
        cluster = model_info.get("cluster")
        virtual = bool(model_info.get("virtual", False))
        # A distributed, clustered, or virtual entry does not own a single
        # local model tree. Do not advertise the local/cache removal endpoint
        # for it, even if its source metadata otherwise looks removable.
        removal_kind = (
            None
            if distributed or cluster or virtual
            else _model_removal_kind(source_type)
        )
