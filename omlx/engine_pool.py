...
class EnginePool:
    ...
    def __init__(
        ...
        Note:
            Pre-load admission consults the process memory enforcer via
            the ``_get_final_ceiling`` callback set by
            ``server.init_server()``. Until the callback is wired up the
            pool admits unconditionally (ceiling==0 short-circuit).
        """
