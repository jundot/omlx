...
and vlm.py), not on the MLX inference thread -- an earlier version of this
docstring claimed otherwise. Every mutation therefore goes through the
cache's own lock (``ExpertLRUCache.resize`` / ``clear``), so the event loop
and the inference thread cannot interleave inside one update.
"""
...
class ExpertResidencyGovernor:
    ...
    def _apply(self, cap: int, per_layer: int) -> None:
        """Retarget the cache capacity.

        Delegates to ``cache.resize()`` when the cache exposes it so the
        whole retarget (capacity, per-layer cap, drain, per-layer trim) is
        one atomic step under the cache lock. The field-by-field fallback
        stays only for duck-typed caches in tests.
        """
        cache = self.cache
        resize = getattr(cache, "resize", None)
        if callable(resize):
            resize(cap, per_layer if self.num_layers > 0 else None)
            return
        cache.capacity = cap
    ...
    def reconcile_per_slot(self, per_slot: int) -> None:
        ...
        then reconciles ``cache.per_expert_bytes`` / ``capacity`` /
