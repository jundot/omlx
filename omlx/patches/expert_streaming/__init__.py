    ...
    if converted and per_expert and budget_bytes > 0:
        try:
            ...
            if _n_projs:
                ...
                if _governor is not None:
                    _governor.reconcile_per_slot(cache.per_expert_bytes)
    ...
    if converted:
        ...
        try:
            ...
            for _lyr in _targets:
                _moe = getattr(_lyr, "mlp", None) or getattr(_lyr, "ffn", None)
                if _moe is None:
                    _block = getattr(_lyr, "block", None)
                    if _block is not None:
                        _moe = getattr(_block, "mlp", None) or getattr(_block, "ffn", None)
                if _moe is not None:
