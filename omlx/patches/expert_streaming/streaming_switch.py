...
# Token-scaled hotness aging for the route_frequency policy. 0 restores the
# insertion-counted window (2 * capacity), which is the fallback and the A/B
# baseline. Default 1024 is the value the mihailescu2m/llama.cpp logs
# calibrate on: "Hotness decay over 1024 tokens -- at 64 an expert
# accumulates ~1.5 uses between halvings, so counters cannot rank and
# eviction degenerates into plain LRU." The claim is checkable and reported
# by RouteFrequencyExpertCache.decay_diagnostics()['mean_uses_at_sweep']:
# near 1.5 means this policy is currently indistinguishable from LRU, and
# the window (not the policy) is what needs to change.
_DECAY_WINDOW_TOKENS = max(
    0, int(os.environ.get("OMLX_EXPERT_STREAMING_DECAY_TOKENS", "1024"))
)
...
class CacheStats:
    ...
    # Per-LAYER stall counters (fork finding, see docs/expert-streaming.md).
    ...
    def decode_stall_rate(self) -> float:
        """Fraction of decode layer-calls that missed at least one expert.

        The north-star for cache work. A policy that halves ``decode_misses``
        while leaving >=1 miss in every layer buys almost nothing; a policy
        that makes some layers miss nothing at all buys real time. Compare
        offline predictions against THIS, never against ``decode_hit_rate``.
        """
        return self.decode_layers_missed / self.decode_layers if self.decode_layers else 0.0

    def prefill_stall_rate(self) -> float:
        return self.prefill_layers_missed / self.prefill_layers if self.prefill_layers else 0.0

    def decode_misses_per_stalled_layer(self) -> float:
        """Mean misses within a layer that missed at all.

        The leverage number: >>1 means misses are *clustered*, so widening
        prefetch (cheap, per the fork) should collapse many of them into the
        same stall. ~1 means misses are spread one-per-layer and no amount of
        prefetch width will help -- only residency will.
        """
        return (
            self.decode_misses / self.decode_layers_missed
            if self.decode_layers_missed
            else 0.0
        )
...
class ExpertLRUCache:
    ...
    def read_failure_stats(self) -> dict[tuple[int, int, str], int]:
        with self._lock:
            return dict(self._read_failures)
    ...
    def resident_bytes(self) -> int:
        ...
        """
        try:
            per_slot = int(getattr(self, "per_expert_bytes", 0) or 0)
...
class RouteFrequencyCache(ExpertLRUCache):
    ...
    def __init__(self, budget_bytes: int, per_expert_bytes: int, num_layers: int | None = None):
        ...
        # Token-scaled aging (see _DECAY_WINDOW_TOKENS). Insertions are the
        # wrong unit: a frozen cache admits nothing (measured: evictions 0,
        # puts == size), so an insertion-counted window can sit at zero
        # through an arbitrarily long decode, and when it does fire its
        # length in tokens depends on the admission rate rather than on
        # anything about the workload.
        self._tokens_seen = 0
        # Diagnostics for the fork's claim: if the mean counter at sweep time
        # is ~1.5, experts accumulate too few uses between halvings for the
        # counters to rank anything and the policy has silently degenerated
        # into plain LRU. That is the number to watch, not the hit rate.
        self.decay_sweeps = 0
        self.decay_uses_sum = 0
        self.decay_uses_n = 0
    ...
    @property
    def _decay_window(self) -> int:
        """Aging window in layer-calls.

        One decoded token produces roughly one layer-call per MoE layer, so
        a window of N *tokens* is N * num_layers layer-calls. Expressing the
        knob in tokens keeps it comparable with the source measurement; the
        conversion is the only place num_layers enters.
        """
        return _DECAY_WINDOW_TOKENS * max(1, self.num_layers)

    def observe_tokens(self, n: int = 1) -> None:
        """Advance token-scaled aging by n layer-calls (see _decay_window).

        The unit the mihailescu2m/llama.cpp logs argue for: they decay over
        1024 tokens because "at 64 an expert accumulates ~1.5 uses between
        halvings, so counters cannot rank and eviction degenerates into
        plain LRU". Insertions are not a proxy for tokens -- a cache that
        admits nothing never advances one (measured: evictions 0, puts ==
        size), so an insertion-counted window can sit at zero through an
        arbitrarily long decode.

        Prefill layer-calls also advance by 1 rather than by their row count,
        which under-counts prefill tokens. Deliberate: prefill is a small
        fraction of the working set (the same reason scan-resistance measured
        negative), and over-counting it would age out the decode hot set.

        No-op when token aging is disabled (``_DECAY_WINDOW_TOKENS = 0``), so
        callers can call this unconditionally.
        """
        if _DECAY_WINDOW_TOKENS <= 0 or n <= 0:
            return
        with self._lock:
            self._tokens_seen += int(n)
            if self._tokens_seen >= self._decay_window:
                self._tokens_seen = 0
    ...
    def _decay_unlocked(self) -> None:
        # Recorded BEFORE halving: what the counters were worth when they
        # were asked to rank. ~1.5 means ranking is impossible.
        if self._freq:
            self.decay_sweeps += 1
            self.decay_uses_sum += sum(self._freq.values())
            self.decay_uses_n += len(self._freq)
    ...
    def decay_diagnostics(self) -> dict:
        """Mean uses per counter at sweep time — the fork's degeneracy test.

        The two units are reported separately and unambiguously because
        conflating them reads as a bug: ``_tokens_seen`` advances once per
        MoE layer per token, while the knob is expressed in tokens, so a
        raw ``tokens_seen > window_tokens`` at zero sweeps looks like aging
        never fires when in fact the window is ``num_layers`` times larger
        than the number printed next to it.
        """
        with self._lock:
            window_calls = self._decay_window
            return {
                "sweeps": self.decay_sweeps,
                "mean_uses_at_sweep": (
                    self.decay_uses_sum / self.decay_uses_n if self.decay_uses_n else 0.0
                ),
                # Layer-calls: the unit actually compared against the window.
                "layer_calls_seen": self._tokens_seen,
                "window_layer_calls": window_calls,
                # The knob, in tokens, and the same count back in tokens.
                "window_tokens": _DECAY_WINDOW_TOKENS,
                "tokens_est": self._tokens_seen / max(1, self.num_layers),
                "num_layers": self.num_layers,
                # How far from the next sweep, in tokens. Lets an operator see
                # that a 46-token decode cannot reach a 1024-token window.
                "tokens_to_next_sweep": max(
                    0.0,
                    (window_calls - self._tokens_seen) / max(1, self.num_layers),
                ),
            }

    def _get_unlocked(self, key: tuple[int, int, str]) -> Any | None:
        self._bump(key)
        return super()._get_unlocked(key)
    ...
    def _retain_hot_unlocked(self, hot_pairs: set) -> int:
        evicted = super()._retain_hot_unlocked(hot_pairs)
...
class _LayerLoadContext:
    ...
    def close(self) -> None:
        ...
        # Token-scaled hotness aging. One layer-call, not self.positions:
        # positions are routed rows (top_k x batch), so passing it would
        # advance the window top_k times too fast. Duck-typed: only
        # route_frequency carries it, and it self-disables when window is 0.
        observe = getattr(self.cache, "observe_tokens", None)
