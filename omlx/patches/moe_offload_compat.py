...
_LAYER_LAYOUTS = (
    ...
    ("model.layers.{layer}.ffn", ".switch_mlp"),
...
def _inspect(path, signature):
    ...
    count = int(text.get("num_experts") or text.get("n_routed_experts") or 0)
    ...
    for key in ("moe_intermediate_size", "intermediate_size"):
        ...
        for parent_template, projection in _LAYER_LAYOUTS:
            reason = _layout_reason(
                ...
                parent_template,
                projection,
                (count, layers, hidden, intermediate, first_moe, moe_freq),
            )
            if reason is None:
                return True, ""
...
def _layout_reason(store, quant, parent_template, projection, geometry):
    ...
    count, layers, hidden, intermediate, first_moe, moe_freq = geometry
