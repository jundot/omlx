    ...
    def __init__(self, model_dir, model, tok, dsv4, hcm, thr=0.6, policy=None):
        ...
        try:
            ...
            if _os.environ.get("OMLX_DSPARK_HEAD_MXFP8", "1") != "0":
                ...
                mx.eval(_qw, _qs)
                class _MXFP8Head:
                    def __init__(s_, W, S_):
                        s_.W, s_.S = W, S_
                    def __call__(s_, x):
                        return q8m(x, s_.W, s_.S)
                model.lm_head = _MXFP8Head(_qw, _qs)
                import logging as _lg
                _lg.getLogger(__name__).info(
                    "dspark: lm_head -> mxfp8 g32 (S13: 0/1024 argmax flips, KL 2e-5)")
        except Exception as _e:
            import logging as _lg
            _lg.getLogger(__name__).warning("dspark: lm_head requant skipped: %r", _e)
