...
def _is_mtp_batch_eligible(gen_batch: Any) -> bool:
    ...
    if _has_grammar_processors(gen_batch):
        # Multi-request MTP shares one draft/verify cycle across rows; the
        # grammar processor's speculative mode is wired for the singleton
        # cycle only (see _grammar_enter_speculative).
        return False
...
def _ineligibility_reason(gen_batch: Any) -> str:
    ...
    if len(uids) != 1:
        ...
        if _has_grammar_processors(gen_batch):
            return "multi-request MTP keeps grammar-constrained rows on the standard step"
        if not _allows_new_mtp_activation(gen_batch, "_omlx_mtp_batch_state"):
