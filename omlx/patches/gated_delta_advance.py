...
"""No-op as of mlx-vlm v0.6.3 (5a4222a) -- the GatedDeltaNet fixes are now upstream.
...
As of mlx-vlm v0.6.3 (5a4222a) BOTH motivations are gone:

1. The ``mx.contiguous(cache[0])`` write and the ``cache.lengths`` /
   ``take_along_axis`` per-element branch are now in upstream verbatim
   (Qwen3_5GatedDeltaNet.__call__, mlx-vlm v0.6.3 (5a4222a)). Redundant.

2. The ``conv_state.shape[0] != B`` and ``mask.shape[0] != B`` fallbacks this
   patch used to DROP are now load-bearing: v0.6.3's batched ``target_verify``
...
False without touching the class, leaving v0.6.3's GatedDeltaNet intact. The
...
def apply_gated_delta_advance_patch(model: Any = None) -> bool:
    """No-op as of mlx-vlm v0.6.3 (5a4222a). See module docstring for why.

    Kept as a stable entry point so the call sites in
    ``omlx/engine/vlm.py`` and ``omlx/utils/model_loading.py`` need no
    change while flyto soft-forks. Overriding mlx-vlm v0.6.3 (5a4222a)'s
    ...
    """
    logger.debug(
        "gated_delta_advance patch is a no-op on mlx-vlm v0.6.3 (5a4222a) "
