...
# The codec itself lives in ``gdn_state_codec`` so the embedded block layout in
# ``paged_ssd_cache`` can drive the same encoder. These aliases keep this
# module's existing spellings.
_GDN_STATE_DTYPES = gdn_codec.STATE_DTYPES
_GDN_INT8_CODEC = gdn_codec.INT8_CODEC
_GDN_RHT_INT8_CODEC = gdn_codec.RHT_INT8_CODEC
_GDN_RHT_INT16_CODEC = gdn_codec.RHT_INT16_CODEC
_GDN_RHT_SEED = gdn_codec.RHT_SEED
_GDN_BF16_CODEC = gdn_codec.BF16_CODEC
_GDN_INT8_CODECS = gdn_codec.INT8_CODECS
_GDN_INT16_CODECS = gdn_codec.INT16_CODECS
_GDN_INTEGER_CODECS = gdn_codec.INTEGER_CODECS
_GDN_REDUCED_CODECS = gdn_codec.REDUCED_CODECS
_GDN_REQUIRED_ORIGINAL_DTYPE = gdn_codec.REQUIRED_ORIGINAL_DTYPE
_gdn_rht_dimension_supported = gdn_codec.rht_dimension_supported
_gdn_rht_sign_values = gdn_codec.rht_sign_values
...
class BoundarySnapshotSSDStore:
                    ...
                    for k, elem in enumerate(state):
                        ...
                        if _has_zero_dim(elem):
                            ...
                            if self._should_quantize_gdn_state(
                                ...
                                stored, scale, encoding = self._encode_gdn_state(
                                    elem, gdn_checks
                                )
                                arrays[key] = stored
                                ...
                                for suffix, value in encoding.metadata_suffixes().items():
                                    info[f"state_{k}_{suffix}"] = value
                            else:
                                arrays[key] = elem
                else:
                    info["has_state"] = "false"
    ...
    ) -> tuple[Any, Any | None, gdn_codec.GDNEncoding]:
        """Encode an ArraysCache recurrent tensor for sidecar storage.

        Thin wrapper over the shared codec: the sidecar's contribution is the
        deferred ``checks`` list, which it evaluates in one batch once every
    ...
    def _verify_gdn_encode_checks(
        ...
        try:
            ...
            # save() degrades every failure to ``False`` at debug level, so
    ...
    _gdn_rht_forward = staticmethod(gdn_codec.rht_forward)
    _gdn_rht_inverse = staticmethod(gdn_codec.rht_inverse)

    @staticmethod
    def _parse_gdn_encoding(
        info: dict[str, str], state_idx: int, codec: str, tensor: Any
    ) -> gdn_codec.GDNEncoding:
        """Read this sidecar's description of an encoded state element."""
        return gdn_codec.parse_encoding(
            codec,
            info.get(f"state_{state_idx}_{gdn_codec.METADATA_ORIGINAL_DTYPE}"),
            info.get(f"state_{state_idx}_{gdn_codec.METADATA_RHT_SEED}"),
            info.get(f"state_{state_idx}_{gdn_codec.METADATA_RHT_DIM}"),
            tensor,
        )
    ...
    def _decode_gdn_state_raw(
        ...
        codec = info.get(f"state_{state_idx}_{gdn_codec.METADATA_CODEC}")
        if not codec:
            return tensor
        with self._gdn_decode_guard(codec):
            encoding = self._parse_gdn_encoding(info, state_idx, codec, tensor)
            scale_key = f"layer_{layer_idx}_state_{state_idx}__scale"
            scale = None
            if encoding.needs_scale:
            ...
            restored = gdn_codec.decode_state(tensor, scale, encoding, scale_key)
    ...
    _validate_gdn_scale = staticmethod(gdn_codec.validate_scale)
