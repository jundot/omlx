...
final class ModelSettingsScreenVMTests: XCTestCase {
    ...
    func testMovaAneRecommendationUsesSharedProfileFields() throws {
        ...
        let status = try decoder.decode(ANETuningStatusResponse.self, from: data)
        XCTAssertEqual(status.recommendation?.processingTps, 100)
        XCTAssertEqual(status.recommendation?.speedupPercent, 4)
        vm.applyANETuningRecommendation(try XCTUnwrap(status.recommendation))
